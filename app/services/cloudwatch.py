...
class CloudWatchService:
    def __init__(self):
        ...
        self.log_group = get_settings().cloudwatch_log_group
...
| filter {F_PATH} = "{path}" and {F_RESPONSE_FILTER}
...
| filter {F_PATH} = "{path}" and {F_MESSAGE} = "Request completed"
...
| filter {F_STATUS} = {status_code} | sort @timestamp desc | limit 50"""
...
| filter {F_CORRELATION} = "{safe}" | sort @timestamp asc | limit 200"""
    ...
    def get_top_apis(self, hours_back: int = 1, limit: int = 10) -> list[dict]:
        query = f"""fields {F_PATH}, {F_METHOD} | filter {F_RESPONSE_FILTER}
...
| filter {F_PATH} = "{path}" and {F_RESPONSE_FILTER} | sort @timestamp desc | limit 100"""
        return self.query_logs(query, hours_back)
...
| filter {F_LEVEL} = "error" or {F_STATUS} >= 500 | sort @timestamp desc | limit 100"""
        return self.query_logs(query, hours_back)
